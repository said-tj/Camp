# ============================================================
# CÓDIGO PARA GRÁFICA DE DESCONOCIDO_UNO
# ============================================================
...
# Realizar ANOVA para cada umbral (usando Desconocido_uno)
resultados_anova <- datos_filtrados %>%
  group_by(Umbrales_analizados) %>%
  anova_test(Desconocido_uno ~ Condicion)
...
resultados_tukey <- datos_filtrados %>%
  group_by(Umbrales_analizados) %>%
  tukey_hsd(Desconocido_uno ~ Condicion)

print("\nResultados Tukey HSD (comparaciones múltiples):")
print(resultados_tukey)

# Guardar resultados
write.csv(resultados_anova, "resultados_anova_desconocido_uno.csv", row.names = FALSE)
write.csv(resultados_tukey, "resultados_tukey_desconocido_uno.csv", row.names = FALSE)
...
datos_resumen <- datos_filtrados %>%
  ...
  summarise(
    media = mean(Desconocido_uno, na.rm = TRUE),
    se = sd(Desconocido_uno, na.rm = TRUE) / sqrt(n()),
    n = n(),
    .groups = 'drop'
  )
...
p <- ggplot(datos_resumen, aes(x = Umbrales_analizados, y = media, 
  ...
  labs(
    x = "Threshold",
    y = "Desconocido_uno",  # Cambia esta etiqueta según lo que represente
  ...
  theme(
    ...
    # Leyenda profesional
    legend.position = c(0.85, 0.85),  # Posición arriba derecha
  ...
  scale_y_continuous(
    limits = c(-10, 30),  # CORREGIDO: Rango para Desconocido_uno (-6 a 30)
    breaks = seq(-10, 30, 10),
...
for(umbral in c("X1", "X3", "X5", "X6")) {
  ...
  if(nrow(comparaciones_umbral) > 0) {
    ...
    alturas <- c(altura_datos + 3, altura_datos + 6, altura_datos + 9)
...
write.csv(tabla_resumen, "tabla_resumen_desconocido_uno.csv", row.names = FALSE)