...
# Realizar ANOVA para cada umbral (usando Area)
resultados_anova <- datos_filtrados %>%
  group_by(Umbrales_analizados) %>%
  anova_test(Area ~ Condicion)
...
resultados_tukey <- datos_filtrados %>%
  group_by(Umbrales_analizados) %>%
  tukey_hsd(Area ~ Condicion)

print("\nResultados Tukey HSD (comparaciones múltiples):")
print(resultados_tukey)

# Guardar resultados
write.csv(resultados_anova, "resultados_anova_area.csv", row.names = FALSE)
write.csv(resultados_tukey, "resultados_tukey_area.csv", row.names = FALSE)
...
p <- ggplot(datos_resumen, aes(x = Umbrales_analizados, y = media, 
  ...
  labs(
    x = "Threshold",
    y = expression(paste("Area (", mu, "V", cdot, "ms)")),  # Etiqueta con símbolo µ
  ...
  theme(
    ...
    # Leyenda profesional
    legend.position = c(0.15, 0.85),  # Posición arriba izquierda
  ...
  scale_y_continuous(
    limits = c(-4000, 10000),  # CORREGIDO: Rango para Area (-800 a ~6000 con margen)
...
for(umbral in c("X1", "X3", "X5", "X6")) {
  ...
  if(nrow(comparaciones_umbral) > 0) {
    ...
    # Definir alturas base para las barras con más separación (ajustado para escala de Area)
    alturas <- c(altura_datos + 1000, altura_datos + 2000, altura_datos + 3000)
...
tabla_resumen <- datos_resumen %>%
  mutate(
    media_se = sprintf("%.0f ± %.0f", media, se)  # Sin decimales para Area
  ) %>%
...
write.csv(tabla_resumen, "tabla_resumen_area.csv", row.names = FALSE)