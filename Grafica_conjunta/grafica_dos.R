...
# ============================================================
# U DE MANN-WHITNEY (no paramétrico, no pareado)
# ============================================================
u_test <- wilcox.test(clasico_vals, alternativo_vals,
                      paired   = FALSE,
                      exact    = FALSE,
                      correct  = TRUE)
cat("=== U de Mann-Whitney (no pareada) ===\n")
print(u_test)
...
ggplot(df_plot, aes(x = Metodo, y = Media, fill = Metodo)) +
  ...
  labs(
    ...
    subtitle = paste0("U de Mann-Whitney — ", sig_label,
                      "  |  W = ", round(u_test$statistic, 1),
